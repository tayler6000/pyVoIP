...
class VoIPCall():
    ...
    def __init__(self, phone, callstate, request, session_id, myIP, portRange=(10000, 20000), ms = None, sendmode="sendonly"):
        debug(f"{self.__class__.__name__}.{inspect.stack()[0][3]} {callstate}, {session_id}, {myIP}, {portRange}, {ms}, {sendmode}")
        ...
        if callstate == CallState.RINGING:
            ...
            debug(f"{self.__class__.__name__}.{inspect.stack()[0][3]} ({audioPortsAdj} == {self.connections} or {self.audioPorts} == 0) and ({videoPortsAdj} == {self.connections} or {self.videoPorts} == 0)")
